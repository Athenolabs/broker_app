...
def execute(filters=None):
	...
	data=frappe.db.sql("""select cmd.marketing,m.nama,sum(point) 
		...
		where cmd.docstatus=1 and cmd.posting_date between "{}" and "{}" {} """.format(filters.get("from"),filters.get("to"),branch))
	columns=["No Marketing:Marketing/Link:200","Nama:Data:200","Total:Float:200"]
	return columns, data
